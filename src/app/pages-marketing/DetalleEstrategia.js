...
const DetalleEstrategia = () => {
  const history = useHistory();
  const location = useLocation();
  const [idEstrategia, setIdEstrategia] = useState(0);
  const [idCampana, setIdCampana] = useState(0);
  ...
  const cargarEstrategia = () => {
    setMostrarDatos(false);
    setMostrarCargaDatos(true);
    if (location.state.tipo == "0") {
      api
        .get(`detalleEstrategia/${location.state.idEstrategia}`)
        .then((res) => res.data)
        .then((data) => {
          console.log(data);
          setIdEstrategia(parseInt(data["idEstrategia"]));
          if (data["idPlan"] != 0)
            setPlan({
              id: parseInt(data["idPlan"]),
          ...
          if (data["leads"] != 0){
            setListaId(data["leads"]);
            setListaElementos(data["leadsElementos"]);
            setListaFiltros(data["filtros"]);
          }
          setMostrarCargaDatos(false);
          setMostrarDatos(true);
        })
        .catch((err) => alert(err));
    } else if (location.state.tipo == "1") {
      api
        .get(`detalleCampana/${location.state.idEstrategia}`)
        .then((res) => res.data)
        .then((data) => {
          console.log(data);
          setIdCampana(parseInt(data["idCampana"]));
          if (data["idPlan"] != 0)
            setPlan({
              id: parseInt(data["idPlan"]),
              descripcion: data["descripcionPlan"],
        ...
        <div>
          <div>
            <div className="page-header">
              <h3 className="page-title"> Detalle de estrategia </h3>
            </div>
            <div className="row">
...
export default DetalleEstrategia;
